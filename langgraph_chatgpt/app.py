...
async def initialize_session_graph():
    if "graph" not in st.session_state:
        st.session_state.graph = await create_agent()
...
async def main():
    ...
    await initialize_session_graph()
    # OpenAI API 키 확인
        ...
        if uploaded_files:
            with st.spinner("📝 문서 처리 중..."):
                ...
                if retriever_tool and docs_info:
                    # 새로운 그래프 생성 및 저장
                    st.session_state.graph = await create_agent(docs_info, retriever_tool)
